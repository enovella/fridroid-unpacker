 ...
 * Created: 2019/6/11
 * Updated: 2025 for Frida 17.0.0 compatibility
...
function getFunctionName() {
  ...
  if (g_AndroidOSVersion > 4) {
    ...
    var artExports = targetModule.enumerateExports();
    ...
    for (i = 0; i < artExports.length; i++) {
      if (artExports[i].name.indexOf("OpenMemory") !== -1) {
        ...
        if (
          g_AndroidOSVersion >= 10 &&
          artExports[i].name.indexOf("ArtDexFileLoader") !== -1
        )
          continue;
        functionName = artExports[i].name;
        logPrint("[*] Export index: " + i + " -> " + functionName);
    ...
    //android 4
    var dvmModule = Process.getModuleByName("libdvm.so");
    ...
    if (dvmExports.length !== 0) {
      ...
      dvmExports = artModule.enumerateExports();
...
function getg_processName() {
  ...
  var fopenPtr = libc.getExportByName("fopen");
  var fgetsPtr = libc.getExportByName("fgets");
  var fclosePtr = libc.getExportByName("fclose");
...
function checkMagic(dataAddr) {
  // Throws access violation errors, not handled at all.
  let dexMagic = "dex\n"; // [0x64, 0x65, 0x78, 0x0a]
  ...
  let foundStart = magic
    .slice(0, 4)
    .map((i) => String.fromCharCode(i))
    .join("");
  let foundVersion = magic
    .slice(4, 7)
    .map((i) => String.fromCharCode(i))
    .join("");
  ...
  if (
    foundStart === dexMagic &&
    dexVersions.includes(foundVersion) &&
    magic[7] === magicTrailing
  ) {
    // Found a dex
    return {
      found: true,
      ext: "dex",
      sizeOffset: 0x20,
      magicString: foundMagicString,
    };
  } else if (
    foundStart === dexMagic &&
    odexVersions.includes(foundVersion) &&
    magic[7] === magicTrailing
  ) {
    ...
    foundStart === kDexMagic &&
    kDexVersions.includes(foundVersion) &&
    magic[7] === magicTrailing
  ) {
    // Found a compact dex
    return {
...
function dumpDex(moduleFuncName, g_processName) {
  ...
  if (g_AndroidOSVersion > 4) {
    var artModule = Process.getModuleByName("libart.so");
    hookFunction = artModule.getExportByName(moduleFuncName);
  } else {
    try {
      var dvmModule = Process.getModuleByName("libdvm.so");
      hookFunction = dvmModule.getExportByName(moduleFuncName);
    } catch (e) {
      var artModule = Process.getModuleByName("libart.so");
      hookFunction = artModule.getExportByName(moduleFuncName);
    }
